-- =============================================
-- MM2 ULTIMATE HUB v7.0 (Меню переработано)
-- Все разделы видны, добавлена скорость, компактно
-- =============================================
...
MainFrame.Size = UDim2.new(0, 280, 0, 480) -- чуть выше, чтобы вместить всё
...
ScrollingFrame.Size = UDim2.new(1, -12, 1, -82)
ScrollingFrame.Position = UDim2.new(0, 6, 0, 46)
ScrollingFrame.BackgroundTransparency = 1
ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 720) -- увеличил, чтобы все элементы поместились
ScrollingFrame.ScrollBarThickness = 4
ScrollingFrame.ScrollBarImageColor3 = Color3.fromRGB(80, 80, 120)
ScrollingFrame.Parent = MainFrame

-- ===== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ВСЕ ВИДИМЫ) =====
local function CreateSection(title, yPos)
    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(1, 0, 0, 22)
    label.Position = UDim2.new(0, 0, 0, yPos)
    label.BackgroundTransparency = 0.0
    label.BackgroundColor3 = Color3.fromRGB(25, 25, 45)
    label.Text = "▸ " .. title
...
-- =============================================
-- Часть 2: Фарм, телепорты, Noclip, Infinite Jump
-- =============================================
...
-- ===== БЕСКОНЕЧНЫЙ ПРЫЖОК (без изменения высоты) =====
...
-- =============================================
-- Часть 3: SPEED (TextBox), Kill All, Auto Shoot, UI, Events
-- =============================================

-- ===== SPEED (ВВОД ЧИСЛА) =====
local speedFrame = Instance.new("Frame")
speedFrame.Size = UDim2.new(1, 0, 0, 40)
speedFrame.Position = UDim2.new(0, 0, 0, 360) -- теперь в секции MOVEMENT, выше
speedFrame.BackgroundTransparency = 0.0
speedFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 48)
speedFrame.Parent = ScrollingFrame

local speedLabel = Instance.new("TextLabel")
speedLabel.Size = UDim2.new(0.4, 0, 1, 0)
speedLabel.BackgroundTransparency = 0.0
speedLabel.BackgroundColor3 = Color3.fromRGB(30, 30, 48)
...
speedBox.Position = UDim2.new(0.42, 0, 0.15, 0)
speedBox.BackgroundColor3 = Color3.fromRGB(50, 50, 70)
speedBox.BackgroundTransparency = 0.0
...
speedSetBtn.BackgroundTransparency = 0.0
...
speedBox.FocusLost:Connect(function(enterPressed)
    if enterPressed then applySpeed(speedBox.Text) end
end)
...
-- ===== ИНТЕРФЕЙС (НОВОЕ РАСПОЛОЖЕНИЕ) =====
CreateSection("VISUAL", 0)
CreateToggle("ESP + Chams", 26, false, function(s) espEnabled=s; UpdateESP(s) end)
CreateToggle("FullBright", 56, false, function(s) fullbrightEnabled=s; UpdateFullbright(s) end)

CreateSection("FARM", 90)
CreateToggle("Auto Farm", 116, false, function(s) StartAutoFarm(s) end)
CreateToggle("Auto Shoot", 146, false, function(s) ToggleAutoShoot(s) end)

CreateSection("TELEPORT", 180)
CreateButton("К Murderer", 206, Color3.fromRGB(200, 50, 50), function()
    local murderers = GetRolePlayers("murderer")
    if #murderers > 0 then TeleportToPlayer(murderers[1]) end
end)
CreateButton("К Sheriff", 238, Color3.fromRGB(50, 150, 255), function()
    local sheriffs = GetRolePlayers("sheriff")
    if #sheriffs > 0 then TeleportToPlayer(sheriffs[1]) end
end)
CreateButton("В лобби", 270, Color3.fromRGB(100, 100, 100), function()
...
CreateSection("MOVEMENT", 308)
CreateToggle("Noclip", 334, false, function(s) ToggleNoclip(s) end)
CreateToggle("Infinite Jump", 364, false, function(s) ToggleInfiniteJump(s) end)
-- speedFrame вставляется автоматически (создан выше, позиция 360)

CreateSection("KILL", 430)
CreateButton("Kill All", 456, Color3.fromRGB(200, 30, 30), KillAllPlayers)
...
closeBtn.BackgroundTransparency = 0.0
closeBtn.Text = "✕"
...
print("MM2 ULTIMATE HUB v7.0 loaded.")
