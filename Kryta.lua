-- =============================================
-- MM2 ULTIMATE HUB v5.0 (FINAL FIX)
-- Удалены Aimbot/Silent Aim, добавлен ESP на пистолет шерифа
-- Компактное меню, ползунок скорости в MOVEMENT
-- =============================================
...
local function UpdateESP(state)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if isDead and not isMurderer then
            -- Ищем пистолет в Workspace (инструменты, которые не принадлежат живым)
            for _, obj in pairs(Workspace:GetDescendants()) do
                if obj:IsA("Tool") and obj:FindFirstChild("Handle") then
                    ...
                    if string.find(name, "gun") or string.find(name, "revolver") then
                        ...
                        local owner = obj:FindFirstChild("Handle") and obj.Handle:FindFirstChild("Player") -- условно
...
-- =============================================
-- Часть 3: Speed Slider, Kill All, Auto Shoot, UI, Events
-- =============================================

-- ===== SPEED SLIDER (в секции MOVEMENT) =====
local speedFrame = Instance.new("Frame")
speedFrame.Size = UDim2.new(1, 0, 0, 50)
speedFrame.Position = UDim2.new(0, 0, 0, 340) -- располагаем выше
...
local function updateSpeedFromPosition(inputPos)
    ...
    speedLabel.Text = "Speed: " .. tostring(spd)
end
...
-- Speed Slider вставляется сразу после (выше他已 определён)

CreateSection("KILL", 410)
CreateButton("Kill All", 435, Color3.fromRGB(200, 30, 30), KillAllPlayers)
...
print("MM2 ULTIMATE HUB v5.0 loaded.")
