-- =============================================
-- MM2 ULTIMATE HUB v6.0 (Полностью переписан)
-- ВСЕ ЭЛЕМЕНТЫ МЕНЮ ВИДИМЫ, ИСПРАВЛЕНЫ ОШИБКИ
-- =============================================
...
MainFrame.BackgroundTransparency = 0.0  -- Сделали непрозрачным
...
-- ===== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ВСЕ ЭЛЕМЕНТЫ С ВИДИМЫМ ФОНОМ) =====
...
-- Перехватываем запрос на прыжок (работает на мобильных и ПК)
...
speedFrame.Position = UDim2.new(0, 0, 0, 350) -- располагаем в секции MOVEMENT
...
-- ===== ИНТЕРФЕЙС (ВСЕ КНОПКИ И ТОГГЛЫ) =====
CreateSection("VISUAL", 0)
CreateToggle("ESP + Chams", 26, false, function(s) espEnabled=s; UpdateESP(s) end)
CreateToggle("FullBright", 58, false, function(s) fullbrightEnabled=s; UpdateFullbright(s) end)

CreateSection("FARM", 94)
CreateToggle("Auto Farm", 120, false, function(s) StartAutoFarm(s) end)
CreateToggle("Auto Shoot", 152, false, function(s) ToggleAutoShoot(s) end)

CreateSection("TELEPORT", 188)
CreateButton("К Murderer", 214, Color3.fromRGB(200, 50, 50), function()
    local murderers = GetRolePlayers("murderer")
    if #murderers > 0 then TeleportToPlayer(murderers[1]) end
end)
CreateButton("К Sheriff", 248, Color3.fromRGB(50, 150, 255), function()
    local sheriffs = GetRolePlayers("sheriff")
    if #sheriffs > 0 then TeleportToPlayer(sheriffs[1]) end
end)
CreateButton("В лобби", 282, Color3.fromRGB(100, 100, 100), function()
...
CreateSection("MOVEMENT", 322)
CreateToggle("Noclip", 348, false, function(s) ToggleNoclip(s) end)
CreateToggle("Infinite Jump", 380, false, function(s) ToggleInfiniteJump(s) end)
-- speedFrame вставляется автоматически (создан выше, позиция 350)
...
print("MM2 ULTIMATE HUB v6.0 loaded.")
